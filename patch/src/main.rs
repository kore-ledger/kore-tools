...
use crate::error::KorePatchError;

#[derive(Parser, Default, Debug)]
#[clap(version, about = "KORE JSON PATCH generator")]
struct Args {
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let state: Value = serde_json::from_str(&args.state)
        .map_err(|_| KorePatchError::InvalidJSON("state".into()))?;
    let new_state: Value = serde_json::from_str(&args.new_state)
        .map_err(|_| KorePatchError::InvalidJSON("new_state".into()))?;
